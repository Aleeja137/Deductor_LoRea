...
void print_mat_values(int *mat, int *LUT, int n, int m)
{
 ...
 for (i = 0; i < n; i++)
 {
  line_index = LUT[i * 2];
  printf("[");
  for (j = 0; j < m; j++) printf("%d ", mat[line_index * line_len + j]);
  printf("]\n");
 }
}
...
void read_matrix(FILE *stream, int **matrix, int **LUT, int n, int m)
{
 char *line = NULL;
 ...
 while ((read = getline(&line, &len, stream)) != -1 && row < n)
 {
  ...
  while (tok)
  {
   ...
   if (first_tok)
   {
    ...
    n_exceptions = atoi(tok);
    first_tok = 0;
    printf("Number of exceptions: %d\n", n_exceptions); // Check
    // Reserve extra memory needed on matrix

    if (((*matrix) = realloc(*matrix, (n + n_exceptions) * line_len * sizeof(int))) == NULL)
    {
     fprintf(stderr, "Error reallocating memory for matrix\n");
     exit(EXIT_FAILURE);
    }

    // Update lookup table
    (*LUT)[(n_line * 2)] = start_line;
    (*LUT)[(n_line * 2) + 1] = n_exceptions;
    start_line += 1 + n_exceptions;
    n_line += 1;
    exception_lines = 0;
  ...
  // Check if last exception is read, if so, set first_tok to 1
  if (exception_lines == n_exceptions)
   first_tok = 1;
  else
   exception_lines++;
  row++;
...
void read_mat_file(char input_file[], int **mat1, int **LUT1, int **mat2, int **LUT2, int **mat3, int **LUT3, int *n1, int *n2, int *m1, int *m2, int *n3, int *m3)
{
 ...
 read_matrix(stream, mat1, LUT1, *n1, *m1);

 // Read second matrix
 read_matrix_dimensions(stream, n2, m2);
 read_matrix(stream, mat2, LUT2, *n2, *m2);

 // Read MGU matrix
 read_matrix_dimensions(stream, n3, m3);
 read_matrix(stream, mat3, LUT3, *n3, *m3);
...
int main(int argc, char *argv[])
{
 ...
 int *LUT0 = NULL, *LUT1 = NULL, *LUT2 = NULL;
 ...
 read_mat_file(csv_file, &mat0, &LUT0, &mat1, &LUT1, &mat2, &LUT2, &n0, &n1, &m0, &m1, &n2, &m2);
 ...
 if (verbose)
 {
  ...
  print_mat_values(mat0, LUT0, n0, m0);
